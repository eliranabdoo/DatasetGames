...
# X_train, X_valid, y_train, y_valid = train_test_split(X_train, y_train, test_size=(1 - train_size) / 2, random_state=38)
...
freg_fs = Pipeline(steps=[
    ('select_features', SelectKBest(score_func=f_regression, k=6))
])

mi_fs = Pipeline(steps=[
    ('select_features', SelectKBest(score_func=mutual_info_regression, k=6))
])
